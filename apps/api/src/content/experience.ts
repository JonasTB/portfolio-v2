import { experienceSchema } from '@portfolio/contracts';
import type { Experience } from '@portfolio/contracts';

/**
 * Fonte de verdade da experiência profissional (ETAPA 07).
 * Dados fornecidos diretamente pelo dono do portfólio (histórico real de
 * LinkedIn) — cargos, empresas, períodos e tecnologias são precisos.
 * `context`/`impact` são uma redação honesta a partir desses fatos e do
 * texto "Sobre" fornecido; não incluem métricas ou realizações específicas
 * não confirmadas. Ordenado do mais recente para o mais antigo.
 */
